...
### `bt_box.csv`
This CSV file contains the summary statistics of the Bradley-Terry based home team win probability distribution for each league. The 4 leagues are MLB, NBA, NFL, and NHL. The columns of the CSV file are below. Results are derived only from games from the second half of each regular season.
- `league`: League name.
...
### `bt_box.pdf`
This PDF file contains 4 box plots for the Bradley-Terry predicted home team win probability distribution. Each box plot displays the summary statistics for the prediction method's home team win probabilities for a specific league. The 4 leagues are MLB, NBA, NFL, and NHL. The statistics plotted are the min, first quartile, median, third quartile, max, and all outliers are represented as individual points. Results are derived only from games from the second half of each regular season.
...
This PDF file contains a histogram for the Bradley-Terry based home team win probability distribution. The 4 leagues are MLB, NBA, NFL, and NHL. There are 30 equal-width bins used for each league's histogram. The density is used instead of the raw counts because each league has a different total count of games. Results are derived only from games from the second half of each regular season.
...
### `ml_box.csv`
This CSV file contains the summary statistics of the moneyline based home team win probability distribution for each league. The 4 leagues are MLB, NBA, NFL, and NHL. The columns of the CSV file are below. Results are derived only from games from the second half of each regular season.
- `league`: League name.
