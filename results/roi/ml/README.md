...
### `{league}.pdf`
This PDF file contains a line graph for the moneyline based prediction method on the specified league. The 4 leagues are MLB, NBA, NFL, and NHL. There are 10 bins, and they are all equally sized. Each line graph shows the return on investment of always betting on the favorite and the return on investment of always betting on the underdog by bin. The favorite of the game is determined by the moneyline based probabilistic prediction method. The ROI is calculated using the moneyline scores.
