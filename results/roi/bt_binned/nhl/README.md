...
### `results/roi/bt_binned/nhl/bin_{bin}.png`
This PNG file contains a line graph of the Bradley-Terry based prediction for the NHL and for the specified bin. There are 10 bins, and they are all equally sized. The line graph shows the return on investment of always betting on the favorite and the return on investment of always betting on the underdog by season. The favorite of the game is determined by the Bradley-Terry probabilistic prediction method. The favorite may disagree with the implied favorite from moneyline scores. However, the ROI is still calculated using the moneyline scores. If the figure does not exist, that means there were no games in the NHL where the Bradley-Terry prediction method had a prediction within the bin for any season. 