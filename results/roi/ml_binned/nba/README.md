...
### `bin_{bin}.pdf`
This PNG file contains a line graph of the moneyline based based prediction for the NBA and for the specified bin. There are 10 bins, and they are all equally sized. The line graph shows the return on investment of always betting on the favorite and the return on investment of always betting on the underdog by season. The favorite of the game is determined by the moneyline based probabilistic prediction method. The ROI is calculated using the moneyline scores. If the figure does not exist, that means there were no games in the NBA where the moneyline based prediction method had a prediction within the bin for any season.
